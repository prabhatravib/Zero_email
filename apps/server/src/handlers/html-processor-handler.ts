// HTML Processor handler with lazy loading
// This module handles HTML sanitization and processing operations

import { getSanitizeHtml } from '../lib/lazy-modules';
import * as cheerio from 'cheerio';

// @ts-expect-error - CSS sanitizer module may not have proper types
...
export class HtmlProcessorHandler {
  /**
   * Preprocess email HTML with sanitization
   */
  async preprocessEmailHtml(html: string): Promise<string> {
  ...
  /**
   * Apply email preferences (theme, image loading)
   */
  ...
  /**
   * Process email HTML with full pipeline
   */
  async processEmailHtml(options: ProcessEmailOptions): Promise<{
    processedHtml: string;
    hasBlockedImages: boolean;
  }> {
    const preprocessed = await this.preprocessEmailHtml(options.html);
    return this.applyEmailPreferences(preprocessed, options.theme, options.shouldLoadImages);
  }
  ...
  /**
   * Sanitize HTML with default safe options
   */
  async sanitizeHtmlSafe(html: string): Promise<string> {
    const sanitizeHtmlModule = await getSanitizeHtml();
    
    return sanitizeHtmlModule.default(html, {
      allowedTags: sanitizeHtmlModule.defaults.allowedTags,
      allowedAttributes: sanitizeHtmlModule.defaults.allowedAttributes,
      allowedSchemes: ['http', 'https', 'mailto', 'tel'],
    });
  }
}

// Factory function to create HTML processor handler
export function createHtmlProcessorHandler(): HtmlProcessorHandler {
  return new HtmlProcessorHandler();
} 